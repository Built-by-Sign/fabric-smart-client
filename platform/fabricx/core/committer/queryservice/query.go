...
// GetTransactionStatuses returns the status of each requested transaction in a
// single batched query. The result is keyed by transaction ID; transactions the
// committer does not yet know about are omitted from the map (the caller treats
// a missing entry as "not final yet"). Correlation uses TxStatus.Ref.TxId so the
// caller does not depend on response ordering.
